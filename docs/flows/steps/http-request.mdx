...
Status codes that must **not** be treated as a failure. This is the inverse of a retry-on list: a code named here stops the retry loop and lets the run continue.
</ParamField>

So the way to tolerate an expected 404 is `"ignore_response_codes": [404]`. Note that the step still records the error rather than a result, so nothing downstream can read the response.
