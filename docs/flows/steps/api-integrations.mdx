...
<Warning>
The **Response Map** tab is present in this step's editor but is not applied to an integration step at run time — only Function and nested-flow steps have their response mapped. Reshape the result in a following [Map step](/flows/steps/map) instead.
</Warning>
