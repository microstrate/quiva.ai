...
<Info>
**Agent-Centric Flows**: Start here. Most Quiva flows begin with an Agent step. Attach tools (connectors) to let agents access your systems, and add other steps only when you need explicit control over branching, transformations, or integrations.
</Info>
...
<ParamField path="provider" type="enum" required>
  ...
  **Options**:
  - `Workforce` - Quiva's custom model (recommended)
  - `OpenAI` - ChatGPT models
  - `Anthropic` - Claude models
  - `Google` - Gemini models
  
  <Tip>Workforce is optimized for Quiva and includes built-in cost optimization</Tip>
</ParamField>
...
<Info>
**Tools = Connectors**: In Quiva, tools and connectors are the same thing - MCP servers that agents can use. Find them in the Marketplace or create custom ones.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Overview" icon="robot" href="/agents/overview">
    Learn more about agents in Quiva
  </Card>
