...
Stream triggers are only available in Advanced Mode and require streams to be configured in the Quiva hosting dashboard.
...
Create a stream in the Quiva hosting dashboard. Configure gateway mappings to write messages to the stream. Add a Stream trigger to your flow. Messages written to the stream automatically trigger your flow. Flow receives message data and processes it in real-time.
...
Streams require gateway mappings to write messages. Configure in Quiva hosting dashboard.
