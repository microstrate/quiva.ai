...
When you add an Upload trigger, quiva.ai provides a unique upload endpoint. Users can upload files directly to this endpoint, triggering your flow with the file contents.

---

## How It Works

Add an Upload trigger to your flow. quiva.ai generates a unique upload endpoint URL. Users upload files to this endpoint via form, API, or embed. Your flow receives the uploaded file and its metadata. Flow processes the document automatically.
...
When you add the trigger, quiva.ai automatically generates a unique upload endpoint. Click "Copy Upload URL" in the trigger configuration. Use this URL in your upload forms or applications. The URL is stable and doesn't change unless you regenerate it.
...
```html
<form action="https://uploads.quiva.ai/YOUR_UPLOAD_ID" 
      method="POST" 
...
async function uploadFile(file) {
  ...
  const response = await fetch(
    'https://uploads.quiva.ai/YOUR_UPLOAD_ID',
    {
      method: 'POST',
      body: formData,
      headers: {
...
```bash
curl -X POST https://uploads.quiva.ai/YOUR_UPLOAD_ID \
