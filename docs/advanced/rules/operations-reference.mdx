...
<Warning>
**If an operator is not on this page, it does not exist.** A rule naming one is
rejected before the step runs, and the step fails with the rule and the operator
named — `unknown operator "arrayLength"`. That is the one rules mistake you are
told about; several others are still silent. See [when a rule produces
nothing](/advanced/rules/core-concepts#when-a-rule-produces-nothing).
</Warning>
...
<Warning>
A pattern beginning with `@` is read as a dynamic value before `regex` ever sees
it, so `"@(.+)$"` fails the step with `unknown operator "(.+)$"`. Write it as
`"[@](.+)$"`.
</Warning>
...
The three date operators read spelled-out units differently, so it is worth
knowing which one you are in:

- **`dateDiff` accepts them properly.** `"days"`, `"months"`, `"minutes"` and the
  rest all work, singular or plural, alongside the letters. `"ms"` is also
  accepted, for milliseconds.
- **`addDate` and `subtractDate` do not.** They scan the string for a digit
  followed by one of the seven letters and ignore the rest, so `"2days"` is two
  days by accident and `"2months"` is **two minutes** — the scan stops at the `m`.
  Write `"2M"`, or capitalise it as `"2Months"`. A form the scan does not match
  at all, `"2 days"` or `"2fortnights"`, is skipped and the date comes back
...
Result: `59`. The same call with `"months"` returns `2`, and with `"ms"` returns
`5097600000`.
...
plausible operator. None of them exists, and each one fails the step it appears
in rather than producing a wrong answer.
