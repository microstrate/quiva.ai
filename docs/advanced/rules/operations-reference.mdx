...
<Warning>
A unit none of them recognises — `"2fortnights"`, or `"2 days"` with a space — is
skipped silently. `addDate` and `subtractDate` return the date unchanged and
`dateDiff` drops the fact, in both cases with no error. Check the spelling.
</Warning>
