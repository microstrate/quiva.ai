...
The operator names are looked up in a slightly narrower table than the main
registry: `expression`, `exp`, `not`, `!`, `generateArray`, `sort`, `sortString`,
`split`, `empty`, `notEmpty`, `concat-array` and `numberFormat` cannot appear as
infix operators. An unrecognised name drops the fact.
