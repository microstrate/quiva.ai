...
<Info>
The pipe operator (`|`) is a Quiva custom extension not found in standard JSONPath implementations.
</Info>
...
**Solutions:** Remove spaces around pipes, check all referenced values exist, and remember that `|` is a Quiva custom extension (not standard JSONPath).
  </Accordion>
...
```json Quiva Equivalent
// Quiva uses $.NODE_ID.path
"$.step1.customer.name"
...
<Info>
**Key Difference:** Quiva uses standard JSONPath syntax with custom extensions, making it more powerful for complex data transformations.
</Info>
...
<Tip>
**Bookmark this page!** This reference guide contains everything you need for JSONPath mapping in Quiva flows.
</Tip>