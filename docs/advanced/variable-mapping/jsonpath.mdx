...
Use `typeof @ === 'string'`, or `Array.isArray` in an Eval step.
...
`String`, `Number`, `Object` and friends are not in scope inside a filter body.

```
$.users[?(String(@property).indexOf('0')>-1)]
