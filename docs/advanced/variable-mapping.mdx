...
Map data between nodes in your flows using JSON Path expressions. Quiva's mapping system allows you to reference data from previous nodes and transform it as needed.
...
- Recursive: `$..email`
- Concatenate: `text|$.NODE.value` (Quiva custom)
- Join: `$.NODE.first| |$.NODE.last` (Quiva custom)
- Fallback: `$.NODE.value|default` (Quiva custom)

**Key points:**
- Arrays are 0-based: `[0]` is first, `[-1]` is last
- Use `===` for equality in filters, not `=`
- Everything is case-sensitive
- Quiva auto-detects `$.` anywhere in your mapping
...
Quiva extends standard JSON Path with a pipe operator (`|`) that allows you to concatenate static text with dynamic values.
...
Quiva uses [jsonpath-plus](https://www.npmjs.com/package/jsonpath-plus) v1.1.0, which supports standard JSONPath expressions:
...
Quiva automatically detects JSON Path expressions anywhere in your mapping by looking for the `$.` pattern. This means you can use JSON Path in:
...
Quiva automatically caches JSONPath expressions, so repeated use of the same path is efficient. This means you can safely reference the same node multiple times without performance penalty:
...
| `text\|$.NODE.prop` | Concatenation (Quiva) | `Hello \|$.user.name` | String |
| `$.A\| \|$.B` | Join with separator (Quiva) | `$.first\| \|$.last` | String |
| `$.NODE.prop\|default` | Fallback value (Quiva) | `$.user.name\|Guest` | String |
...
This documentation is based on **jsonpath-plus version 1.1.0**, which Quiva uses for JSON Path mapping. Key features of this version include:
...
**Quiva Platform Version:** Current
