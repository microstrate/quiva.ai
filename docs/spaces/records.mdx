...
<Warning>
**Task actions rewrite the task's status.** Adding or updating any task action recomputes the parent task's status from its checklist: none done becomes `to_do`, some done becomes `in_progress`, all done becomes `completed`. Chat actions are open-ended and are excluded from the count.

Those three ids are written as they are, and they are not checked against the Space's own status list. In a Space whose statuses use different ids, a task can end up in a status the Space does not define — stored, but matching no filter and rendering as raw text. If you use form or document checklists, give the Space statuses with those three ids.
</Warning>
</content>
