...
<Warning>
**Neither card kind offers a Visible rule, and that is deliberate.** Hiding a section by hiding the
card around it leaves the answers inside it in the record and still subject to schema validation.
To make a section conditional, put the rule on the **row** that contains the card — and keep the
conditional fields in ordinary rows rather than inside the card. The full explanation, with what
goes wrong, is in
