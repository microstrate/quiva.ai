...
This guide compiles proven patterns, strategies, and lessons learned from building successful assistants. Follow these practices to create assistants that perform reliably, cost-effectively, and delight users.
...
Each assistant should have one clear purpose.

<Tabs>
  <Tab title="Good: Focused Assistants">
    ```
    Assistant 1: Customer Service
    - Answer product questions
    - Handle order inquiries
    - Process simple refunds
    
    Assistant 2: Lead Qualification
    - Qualify inbound leads
    - Enrich company data
    - Book demos
    
    Assistant 3: Content Generator
  ...
  <Tab title="Bad: Swiss Army Knife Assistant">
    ```
    Assistant: Everything Bot
...
<AccordionGroup>
  <Accordion title="Define Success Clearly" icon="bullseye">
    ...
    When assistants know what success looks like, they optimize for it.
  </Accordion>
  ...
  <Accordion title="Show, Don't Tell" icon="lightbulb">
    ...
    Assistant: "I completely understand your frustration—waiting 
  ...
  <Accordion title="Spell Out Edge Cases" icon="triangle-exclamation">
    Don't assume assistants will "figure it out." Explicitly handle edge cases.
...
<AccordionGroup>
  <Accordion title="Search Before You Answer" icon="magnifying-glass">
    For knowledge-based assistants, always search first.
    ...
    Wrong: [Assistant answers from memory - might be outdated]
  ...
  <Accordion title="Enrich Before You Qualify" icon="sparkles">
    For sales assistants, gather data before making decisions.
  ...
  <Accordion title="Escalate When Uncertain" icon="arrow-up-right-from-square">
    When assistants hit their limits, escalate gracefully.
...
<AccordionGroup>
  ...
  <Accordion title="Reduce Unnecessary Tool Calls" icon="brain">
    Every extra tool call or exchange costs tokens.
  ...
  <Accordion title="Cache Common Queries" icon="database">
    ...
    **Approach:**
    ...
    5. Fall back to assistant for unique queries
...
<AccordionGroup>
  <Accordion title="Choose the Right Model" icon="sliders">
    Match model capability to task complexity. The Provider tab's model picker always reflects
    the current catalogue, so choose from what's listed there rather than a name in this guide —
    model options change over time.
    ...
    **General guidance:**
    ...
    - Complex reasoning or high-stakes tasks: step up to a larger, more capable model only when
      quality actually demands it.
  ...
  <Accordion title="Watch Your Spend" icon="bell">
    Track costs regularly so nothing surprises you.
...
Before rolling out to production, test:
...
<AccordionGroup>
  <Accordion title="Over-Engineering Before Validation" icon="exclamation">
    **Mistake:** Building complex, feature-rich assistants before testing basic functionality.
  ...
  <Accordion title="Not Handling Tool Failures" icon="wrench-simple">
    ...
    **Fix:** Explicitly instruct assistants how to handle tool failures.
  ...
  <Accordion title="Vague Success Criteria" icon="question">
    **Mistake:** "The assistant should be helpful" with no concrete metrics.
    
    **Fix:** Define measurable success criteria before rolling out.
  ...
  <Accordion title="Optimizing Prematurely" icon="gauge-high">
    **Mistake:** Spending hours optimizing token usage before validating the assistant works.
...
<Steps>
  ...
  <Step title="Limited Users (Week 2-3)">
  ...
  <Step title="Full Availability (Week 7+)">
...
<Steps>
  ...
  <Step title="Friday: Save & Monitor">
    - Save improvements
...
Analyzing top-performing assistants reveals common patterns:
...
<CardGroup cols={2}>
  ...
  <Card title="Clear Escalation Path" icon="arrow-up">
    Assistants know when and how to escalate. No guessing.
  </Card>
...
Use this checklist when building or optimizing assistants:
